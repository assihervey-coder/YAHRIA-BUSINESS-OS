...
export async function GET(req: NextRequest) {
  return withAuth(req, null, async (s) => {
    ...
    const treasury = accounts.reduce((s2, a) => s2 + a.balance, 0)
    const openInvoices = invoices.filter((i) => ['SENT', 'OVERDUE', 'PARTIAL'].includes(i.status))
    const receivables = openInvoices.reduce((s2, i) => s2 + (i.total - i.paidAmount), 0)
    const overdue = invoices.filter((i) => i.status === 'OVERDUE')
    const overdueAmount = overdue.reduce((s2, i) => s2 + (i.total - i.paidAmount), 0)
    const payables = expenses.filter((e) => e.status !== 'PAID').reduce((s2, e) => s2 + e.amount + e.vatAmount, 0)
    const monthStart = new Date(new Date().getFullYear(), new Date().getMonth(), 1)
    const caMonth = invoices.filter((i) => i.status !== 'DRAFT' && i.status !== 'CANCELLED' && i.issueDate >= monthStart).reduce((s2, i) => s2 + i.total, 0)
    const vatCollected = invoices.filter((i) => i.status !== 'DRAFT' && i.status !== 'CANCELLED').reduce((s2, i) => s2 + i.vatAmount, 0)
    const vatDeductible = expenses.reduce((s2, e) => s2 + e.vatAmount, 0)
    ...
    for (let d = 13; d >= 0; d--) {
      ...
      const inSum = moved.filter((p) => p.direction === 'IN').reduce((s2, p) => s2 + p.amount, 0)
      const outSum = moved.filter((p) => p.direction === 'OUT').reduce((s2, p) => s2 + p.amount, 0)
    ...
    return {
      ...
      session: { name: s.name, role: s.role },
      kpis: {
